...
BC_plot <- ggplot(bcdf, aes(x = NMDS1, y = NMDS2)) +
  ...
  annotate("text", x = -0.02, y = -0.02, label = "Stress = ") +
  annotate("text", x = -0.008, y = -0.02, label = round(ord.nmds.bray$stress, 4)) +
...
UWF_plot <- ggplot(wuni, aes(x = NMDS1, y = NMDS2)) +
  ...
  geom_point(aes(color = Sample_type)) +
  stat_ellipse(aes(color = Sample_type)) +
  annotate("text", x = -0.32, y = -0.28, label = "Stress = ") +
  annotate("text", x = -0.19, y = -0.28, label = round(ord.nmds.uni$stress, 4)) +
...
ggsave("../Results/4)Differential_Abundance_heatmap_extra.pdf",width = 11, height = 8)
```

# Supplementary 2 - Calculate some stats re deseq2 enrichment
```{r 14}
...
data_long <- gather(mytable, condition, Comparison, ConditionA:ConditionC, factor_key = TRUE)
joined2 <- sigdat %>%
  full_join(data_long)
...
dat_long <- joined2 %>%
    gather("count", "measurement", count, Phycount)
