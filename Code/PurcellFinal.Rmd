...
```{r 1}
# load data
ps0  <- readRDS("data/phyloObject.rds")
...
saveRDS(rarefaction_curve_data, file = "Purcell Final/PrimaryData/rare_object.rds")
```
...
rarefaction_curve_data <- readRDS(file = "Purcell Final/PrimaryData/rare_object.rds")
summary(rarefaction_curve_data)
...
ggsave("Purcell Final/Results/1)Rarefaction_Curve.pdf", width = 11, height = 8)
```
...
ggsave("Purcell Final/Results/2)Alpha_Diversity.pdf", width = 7, height = 4.5)
```
...
ggsave("Purcell Final/Results/3)Beta_Diversity.pdf", width = 6, height = 4.5)
```
...
ggsave("Purcell Final/Results/4)Relative_Abundance.pdf", width = 7, height = 8)
```
...
write.csv(SignificantResults, file = "Purcell Final/Results/SignificantFoldChangeResults.csv")
```
...
ggsave("Purcell Final/Results/5)Differential_Abundance_heatmap.pdf", width = 8, height = 8)
```
...
ggsave("Purcell Final/Results/6)Differential_Abundance_heatmap_extra.pdf",width = 11, height = 8)
```

# Supplementary Differential Abundance Figure  
```{r 12}
...
ggsave("Purcell Final/Results/S1)Differential_Abundance_clinVSself.pdf", width = 7, height = 4)
...
ggsave("Purcell Final/Results/S1)Differential_Abundance_swabsVSstool.pdf", width = 12, height = 8)
```
...
ggsave("Purcell Final/Results/S2)Sanity_FoldChange_plots.pdf", width = 7, height = 8)
...
ggplot(sanity, aes(Sample_type, Abundance)) + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("Purcell Final/Results/S2)Sanity_logAbundance_plots.pdf", width = 7, height = 8)
```
